...
const WORK: Entry[] = [
  {
    ...
    bullets: [
      ...
      "Cut a customer-facing summary endpoint's cold-cache p99 from 12s+ to sub-second (>95%) by rewriting per-row correlated subqueries into a parallelized two-pass query pipeline with counts recombined at the application layer — equivalence proven via SQL snapshot tests.",
      "Diagnosed an 18.4s p99 on a usage endpoint to a non-sargable timestamp predicate scanning full org history; a DST-safe sargable bound across 14 query sites enabled a combined-index plan — ~5× fewer disk block reads, ~90–420ms, provably identical results.",
      "Built server-side RBAC for the customer portal: JWT token-exchange with server-resolved identity, an explicit roles × resources policy matrix, and row-level ownership enforcement across sessions, billing, certificates, and support.",
      "Developed the FastAPI billing API surface behind the customer portal (invoices, payment sources, subscriptions, usage) with dual authentication for internal users and API clients.",
      "Built an automated post-invoice reconciliation engine performing session-level diffs to detect and correct over- and under-billing after invoice closure.",
      "Audited the full population of production prepaid usage records against billed sessions (no sampling), including timezone-boundary correctness.",
      "Automated multi-tenant enterprise billing: master–child invoicing with tiered pricing, aggregate contractual-minimum evaluation, and automatic true-up invoice generation.",
